...
module.exports = function(file, opts, callback) {
    ...
    var compassExecutable = 'compass',
        ...
        relative_file_name = path.relative(path.join(opts.project_path || process.cwd(), opts.sass_dir), file);
    ...
    if (process.platform === 'win32') {
        options.push(opts.project_path.replace(/\\/g, '/'));
    } else {
        options.push(opts.project_path);
    }
    ...
    if (opts.config_file) {
        ...
        if (opts.generated_images_dir) {
            options.push('--generated-images-path', opts.project_path + '/' + opts.generated_images_dir);
        }
    ...
    var child = spawn(compassExecutable, options, {cwd: opts.project_path || process.cwd()}),
        stdout = '',
        stderr = '';
    if (opts.logging) {
    ...
    child.on('close', function(code) {
        var new_path = path.join(opts.project_path, opts.css_dir, relative_file_name);
